	...
	$(".jian").click(function() {
		...
		if(quantity == 1) {
			if(confirm("您是否真要删除该条目？")) {
				location = "/ssh_goods/deleteCartItemById?cartItemId="+cartItemId;
			}
		} else {
			$.post(
					"/ssh_goods/reduceQuantity",
...
function DeleteCart(){
	var cartItemIds="";
	$(".checkedBox").each(function(){//获得复选框这个类遍历
	var cartItemId = $(this).attr("value");//遍历拿到cartItemId
	if($(this).is(':checked')){//判断这个是否被选中
		cartItemIds=cartItemIds+cartItemId+",";//如果选中就将ID放入a中
	}
	});
	location="/ssh_goods/DeleteCart?cartItemIds="+cartItemIds;//携带a数组到后端
}
...
function listCart(){
	...
	location="/ssh_goods/listCartItemsById?cartItemIds="+cartItemIds;//携带a数组到后端
}
</script>
  </head>
  <body>
...
<c:forEach items="${clist}" var="c">
					<tr align="center">
						<td align="left"><input id="${c.cartItemId}" value="${c.cartItemId}"
							type="checkbox" name="checkboxBtn" checked="checked" class="checkedBox"/></td>
						<td align="left" width="70px"><a class="linkImage"
							href="<c:url value='/getBookByBid?book.bid=${c.book.bid}'/>"><img border="0"
								width="54" align="top"
								src="<c:url value='/${c.book.image_b }'/>" /></a></td>
						<td align="left" width="400px"><a
							href="<c:url value='/getBookByBid?book.bid=${c.book.bid}'/>"><span>${c.book.bname }</span></a>
						</td>
						<td><span>&yen;<span class="currPrice"
								id="${c.cartItemId}CurrPrice">${c.book.currPrice }</span></span></td>
						<td><a class="jian" id="${c.cartItemId }Jian"></a>
							<input class="quantity" readonly="readonly" id="${c.cartItemId}Quantity" type="text" value="${c.quantity }" />
							
							<a class="jia" id="${c.cartItemId }Jia"></a></td>
						<td width="100px"><span class="price_n">&yen;<span
								class="subTotal" id="${c.cartItemId }Subtotal">${c.subtotal }</span></span>
						</td>
						<td><a href="<c:url value='/deleteCartItemById?cartItemId=${c.cartItemId }'/>">删除</a></td>
					</tr>
				</c:forEach>
	
	<tr>
		<td colspan="4" class="tdBatchDelete">
			<a href="javascript:DeleteCart()">批量删除</a>
		</td>
	...
	<tr>
		<td colspan="7" align="right">
			<a href="javascript:listCart()" id="jiesuan" class="jiesuan"></a>
		</td>
	</tr>
</table>
