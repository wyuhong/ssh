...
function validateLoginname(){
	...
	if(!value){
		...
		//符合不为空条件并且符合长度在6-20之间，那么就可以进行ajax校验
		$.post(
				"/ssh_goods/existLoginname",
				{"loginname":value},
				function(data){
					if(data.status=="exist"){
						/*//数据库中已经存在该用户登录名
						var lableId = $("#loginname").attr("id")+"Error";
						$("#"+lableId).text("登录名已被占用")
						showHideError($("#"+lableId));*/
						showHideErrorByLableId($("#loginname"),"登录名已被占用");
						$("#"+$("#loginname").attr("id")+"Error").removeClass("successClass");
					}else{
						/*//数据库不存在登录名，可以注册
						var lableId = $("#loginname").attr("id")+"Error";
						$("#"+lableId).text("登录名可以注册")
						showHideError($("#"+lableId));*/
						showHideErrorByLableId($("#loginname"),"登录名可以注册");
...
function validateEmail(){
	...
	if(!value) {// 非空校验
		showHideErrorByLableId($("#email"),"Email不能为空！");//正则表达式
	} else if(!(/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/.test(value))){//格式校验
		showHideErrorByLableId($("#email"),"Email格式不正确！");
	}else {//校验数据库中是否含有该Email,是否被注册过
		$.post(
				"/ssh_goods/existEmail",
				{"email":value},
				function(data){
					if(data.status=="exist"){
						showHideErrorByLableId($("#email"),"邮箱已被注册");
						$("#"+$("#email").attr("id")+"Error").removeClass("successClass");
					}else{
						showHideErrorByLableId($("#email"),"邮箱可以使用");
...
$(function(){
	...
	//当光标放入执行
			function(){
				$(this).attr("src","/ssh_goods/images/regist2.jpg")
			},
	//当光标离开执行
			function(){
				$(this).attr("src","/ssh_goods/images/regist1.jpg")
			}
	);
